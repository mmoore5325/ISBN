def valid_isbn_10?(isbn)
	...
	# isbn.to_s.chars.map(&:to_s)
	# if isbn.last != "x" || isbn.last != "X"
	# 	isbn = isbn.map {|i| i.gsub(/[^\d]/, '')}
	# end

	# isbn.each_with_index do |value, idx|
	# 	if value == ""
	# 		isbn.delete_at(idx)
	# 	end
	# end
...
# 	checkdigit = checkdigit.isbn.to_s.chars.map(&:to_s)

# 	if checkdigit != "x" || checkdigit != "X"	#if the check digit is x or X...
# 		checkdigit = checkdigit.map {|i| i.gsub(/[^\d]/, '')}
# 	elsif checkdig == "x" || checkdigit == "X"
